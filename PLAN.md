...
- Add final result ordering and limiting across the query shapes supported by Milestones 1-4.
- Add lightweight optimizer rules for filter pushdown and projection pruning without changing query semantics.
- Improve end-to-end fixture coverage and CLI/demo documentation so supported query patterns are easy to validate manually.
- Support `ORDER BY` and `LIMIT` for scan-only, aggregate, join, and aggregate-over-join queries.
- Keep `OFFSET`, external/spilling sort, indexes, cost-based optimization, join reordering, and persistent statistics out of scope.

### Task 1: SQL Surface Expansion

- Extend the SQL AST and parser to represent `ORDER BY` and `LIMIT` in the final query clause order.
- Support one or more `ORDER BY` items, each with optional `ASC` or `DESC`.
- Default ordering direction is `ASC`.
- Define null ordering as `NULLS LAST` for both ascending and descending order in this milestone.
- Support `LIMIT` with a non-negative integer literal only.
- Reject unsupported forms such as `OFFSET`, `NULLS FIRST/LAST` syntax, expressions that cannot be typed, and negative limits with structured errors.

### Task 2: Ordering and Limit Binding

- Bind `ORDER BY` expressions against the final projection scope when possible, including projection aliases.
- Allow `ORDER BY` to reference projected columns and deterministic scalar expressions over available output columns.
- Reject ambiguous or unavailable column references in `ORDER BY` with bind errors.
- Bind `LIMIT` as an integer plan value and reject values that cannot fit in the engine integer type.
- Ensure `ORDER BY` and `LIMIT` do not change `ResultSet::schema()`.

### Task 3: Logical Plan Changes

- Add logical `Sort` and `Limit` nodes.
- Compose final plans as:
  `base relational plan -> Projection -> Sort(optional) -> Limit(optional)`.
- Keep `Sort` after projection so alias-based ordering is straightforward and output rows match final schema.
- Allow `Limit` without `Sort`; such queries preserve upstream deterministic execution order.
- Ensure sort/limit nodes can wrap scan-only, aggregate, join, and aggregate-over-join query plans.

### Task 4: Physical Sort and Limit Execution

- Implement an in-memory stable sort over materialized output rows or output chunks.
- Compare values by logical type, using existing numeric coercion rules for `BIGINT`/`DOUBLE` comparisons.
- Support `BOOLEAN`, `BIGINT`, `DOUBLE`, and `VARCHAR` ordering.
- Treat `NULL` as greater than all non-null values so nulls appear last.
- Implement `LIMIT` by truncating rows after optional sorting and before public `ResultSet` materialization.
- Preserve deterministic ordering for equal sort keys by keeping original input order.

### Task 5: Filter Pushdown

- Add a simple optimizer pass that pushes safe `WHERE` predicates closer to scans.
- For single-source queries, push deterministic predicates that reference only scan columns below projection/aggregation where valid.
- For joins, split `AND` predicates and push each single-relation predicate to the matching side before the join; keep multi-relation predicates above the join.
- Never push `HAVING` below aggregation.
- Never push predicates that contain aggregates or unsupported expression forms.
- Preserve a conservative fallback: if a predicate cannot be proven safe to push, leave it in its original position.

### Task 6: Projection Pruning

- Add required-column analysis for scan sources.
- Include columns referenced by `SELECT`, `WHERE`, `JOIN ON`, `GROUP BY`, aggregate arguments, `HAVING`, and `ORDER BY`.
- Prune scan output schemas and chunk materialization to required columns when the scan path supports it.
- Preserve external result schemas and column order exactly as the user requested.
- Use conservative fallback for CSV/JSONL cases that cannot physically skip parsing yet; logical pruning is still useful if physical pruning is not available everywhere.

### Task 7: Optimizer Validation Hooks

- Keep optimizer rules separate from binding so each rule can be tested independently.
- Add a way for tests to inspect or assert optimized logical plan shape without depending on executor internals.
- Ensure optimized and unoptimized execution produce identical public results for representative queries.
- Keep optimization optional internally so future debugging can compare pre- and post-optimization plans.

### Task 8: CLI and Documentation

- Update the CLI demo flow to include examples for `ORDER BY`, `LIMIT`, joins, aggregation, and aggregate-over-join queries.
- Add README examples for the main supported query shapes and qualification rules.
- Include at least one copy-pasteable CSV query and one JSONL query that exercise sorting and limiting.
- Document milestone limits clearly: no `OFFSET`, no external sort, and only conservative pushdown/pruning.

### Task 9: Tests and Fixtures

- Add parser tests for `ORDER BY`, multiple sort keys, `ASC`, `DESC`, `LIMIT`, and invalid limit syntax.
- Add planner tests for alias-based ordering, unavailable order columns, limit binding, `Sort`/`Limit` plan placement, and optimizer rewrites.
- Add executor tests for ascending sort, descending sort, multi-key sort, null ordering, stable equal-key ordering, and limit truncation.
- Add optimizer tests for single-source filter pushdown, join-side filter pushdown, non-pushable predicate retention, and projection pruning required-column sets.
- Add public API tests covering sort/limit over scan-only, aggregate, join, and aggregate-over-join queries.
- Add or extend fixtures with duplicate sort keys, null sort keys, and extra unused columns for pruning tests.

### Acceptance Criteria

- A user can run `SELECT name, age FROM read_csv('fixtures/csv/people.csv') ORDER BY age DESC LIMIT 2`.
- A user can run a joined query with `ORDER BY` and `LIMIT`, and results are deterministic.
- A user can run an aggregate-over-join query with `ORDER BY` and `LIMIT`.
- `ORDER BY` supports projection aliases and returns structured bind errors for unavailable or ambiguous names.
- `LIMIT 0` returns an empty result with the correct schema.
- Filter pushdown and projection pruning preserve identical public results compared with the unoptimized logical plan.
- All Milestone 5 tests pass under `moon test`, and the public API generated by `moon info` reflects only intended surface changes.
